...
1. В `WakeWordListener` включается browser-side `SpeechRecognition`.
2. При детекции wake-word UI переходит в состояние «Слушаю».
3. Команда записывается через `MediaRecorder` с silence endpointing.
4. Аудио отправляется в `POST /api/voice/listen` (multipart `audio`).
5. Backend возвращает транскрипт, он отправляется в чат как обычное сообщение пользователя.
6. После ответа Asya listener возвращается в режим ожидания.

### 8.3. Безопасность и ограничения

- Браузер запрашивает разрешение микрофона явно.
- Аудио не логируется.
- Транскрипт не логируется в production.
- При скрытии вкладки слушание останавливается полностью (покрыто frontend-тестом).
- Поддержка зависит от браузера; для 1.0 целевой браузер — Chrome.
