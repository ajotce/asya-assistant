...
Минимальный frontend набор (MVP) покрывает:
- рендер главного экрана чата;
- отправку сообщения;
- отображение streaming-состояния;
- загрузку и отображение настроек модели;
- редактирование системного промта и сохранение;
- рендер страницы состояния;
- отображение ошибок на страницах чата/состояния.
...
- Frontend unit-тесты (минимальный MVP набор) проходят.
