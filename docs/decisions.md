...
## ADR-019 (1.0.11, этап B): Форма открытой регистрации

Решение:
- Для этапа B после стабилизации этапа A рекомендуемый режим открытия регистрации: `open_with_oauth_only`.
- Email+password открытая регистрация (`open`) остается допустимым fallback режимом, но не рекомендована для первого публичного rollout.

Почему:
- Уменьшается поверхность атаки (нет отдельного email verification/reset flow в первой волне открытия).
- Ниже операционная нагрузка на anti-abuse и поддержку пользователей.
- Проще контроль качества первых публичных пользователей через OAuth identity providers (Google/Yandex).

Последствия:
- В runtime используется `REGISTRATION_MODE` с тремя значениями: `invite_only`, `open_with_oauth_only`, `open`.
- На этапе A default остается `invite_only`.
- Перед переходом в `open` требуется отдельный security review email-flow и anti-abuse мер.
